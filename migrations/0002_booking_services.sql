-- Booking services expansion: variable-length event coverage.
-- The production table is empty; the table is rebuilt with relaxed checks and
-- the service column, and the triggers are recreated (dropping a table drops
-- its triggers). booking_jobs is recreated first so the foreign key target
...
CREATE TABLE booking_requests (
  ...
  package_id TEXT NOT NULL CHECK (package_id IN ('mini', 'standard', 'group', 'event')),
