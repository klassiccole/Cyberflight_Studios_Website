-- Booking storage foundation; not yet connected to the public form.
-- All timestamps are UTC Unix seconds. Times and prices must be server-derived.
CREATE TABLE booking_requests (
  ...
  package_id TEXT NOT NULL CHECK (package_id IN ('mini', 'standard', 'group')),
  ...
  CHECK (session_end - session_start = CASE package_id WHEN 'mini' THEN 1800 ELSE 5400 END),
...
-- A single INSERT checks conflicts and writes the hold under the same DB lock.
-- Expired pending rows stop blocking immediately, even before the expiry job runs.
...
-- Inserted in the same transaction as the request/decision. Delivery is separate.
-- No customer data or private review token needs to be copied into this queue.
