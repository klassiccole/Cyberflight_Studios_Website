...
export function normalizeBooking(input) {
  ...
  if (input.package === 'wedding') {
    ...
    const wEnd = sessionSeconds(input.date, 1410);
...
export function normalizeSignature(input, booking) {
  ...
  // The signature is the payer's: it must match the name entered on the form.
  // The graduate may be someone else; their sign-off is collected at the shoot.
  const norm = value => value.trim().replace(/\s+/g,' ').toLowerCase();
  if (!booking?.details?.name || norm(typedName) !== norm(String(booking.details.name))) throw new BookingError('signature_name_mismatch');
