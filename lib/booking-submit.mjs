...
Contact: ${booking.details.email} · ${booking.details.phone}`,
    ...
    // The booking is already saved and blocks the slot in the database, so a
    // failed calendar write is logged but never fails the request.
    console.error(JSON.stringify({event:'hold_event_failed'}));
  }
}
