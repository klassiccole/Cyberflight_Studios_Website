...
Contact: ${booking.details.email} · ${booking.details.phone}`,
    ...
    return String(error&&error.message||'unknown').slice(0,60);
  }
  return '';
}
...
export function createBookingHandlers({readAvailability,verify=verifyTurnstile,now=()=>Math.floor(Date.now()/1000)}) {
  async function execute(context,mode) {
    ...
    try {
      ...
      const holdNote=await createHoldEvent(env,booking,id,createdAt+HOLD_SECONDS);
      await sendOwnerNotification(env,booking,{id,expiresAt:createdAt+HOLD_SECONDS},holdNote);
