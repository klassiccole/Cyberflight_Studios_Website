...
function pilotGate(env,booking) {
  // The existing contract is unfinished. Use real storage for the owner's trial,
  // but do not collect submissions from other customers until the draft is settled.
  const allowed=env.BOOKING_ALLOWED_EMAIL;
  if(typeof allowed!=='string'||!allowed.trim())throw new BookingError('pilot_email_not_configured',503);
  if(booking.details.email.toLowerCase()!==allowed.trim().toLowerCase())throw new BookingError('booking_not_open_yet',403);
}
  ...
  async function execute(context,mode) {
    ...
    try {
      ...
      try {
        ...
        if((String(error?.message)+' '+String(error?.cause?.message)).includes('booking_overlap'))throw new BookingError('time_unavailable',409);
        throw error;
      }
