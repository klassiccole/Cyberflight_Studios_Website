import {accessToken,googleJSON} from './google-auth.mjs';
...
/* Real booking event on the Bookings Calendar. This is the booking: the
   owner manages it there (delete = cancel), and availability follows. */
async function createBookingEvent(env,booking,id) {
  try {
    const token=await accessToken(env);
    const serviceLabel=booking.package==='event'?'Event Coverage':booking.package==='wedding'?'Wedding':PACKAGES[booking.package].name;
    const isAllDay=booking.package==='wedding';
    const eventDef=isAllDay?{date:booking.date}:{dateTime:new Date((booking.sessionStart-1800)*1000).toISOString(),timeZone:ZONE};
    const eventEndDef=isAllDay?{date:(function(){const d=new Date(booking.date+'T12:00:00Z');d.setUTCDate(d.getUTCDate()+1);return d.toISOString().slice(0,10)})()}:{dateTime:new Date((booking.sessionEnd+1800)*1000).toISOString(),timeZone:ZONE};
    const event={summary:`${serviceLabel} — ${booking.details.name}`,
...
Location: ${booking.details.location}`,
      ...
      extendedProperties:{private:{requestId:id}}};
    ...
    const reason=String(error&&error.message||'unknown').slice(0,60);
    console.error(JSON.stringify({event:'booking_event_failed',reason,stack:String(error&&error.stack||'').split('\n').slice(1,20).join(' | ').slice(0,1200)}));
    return reason;
  }
}

async function sendOwnerNotification(env,booking,receiptRow,holdNote='') {
  ...
  const d=booking.details,isEvent=booking.package==='event',isProposal=booking.package==='event'||booking.package==='wedding';
  const serviceLabel=isEvent?'Event Coverage':isProposal?'Wedding':PACKAGES[booking.package].name;
  ...
  const sessionText=isEvent
    ? `${fmt.format(date)} · ${booking.length/60} hour${booking.length>=120?'s':''} · ${timeLabel(booking.time)} start (Eastern)`
    : isProposal
    ? `${fmt.format(date)} · full day`
    : `${fmt.format(date)} · ${timeLabel(booking.time)}–${timeLabel(booking.time+p.duration)} Eastern`;
  const lines=['CYBERFLIGHT STUDIOS — NEW BOOKING REQUEST','',
    `Request ID: ${receiptRow.id}`,
    `Service: ${serviceLabel}${isEvent?' — quoted by proposal':` — ${money(booking.priceCents)}`}`,
    `Session: ${sessionText}`,
  ...
  try {
    const response=await fetch('https://api.resend.com/emails',{method:'POST',
      ...
      body:JSON.stringify({from:'Cyberflight Studios <cole@cyberflight.studio>',
        ...
        subject:`New ${serviceLabel} request — ${d.name}${holdNote?` [CALENDAR FAILED: ${holdNote}]`:''}`,text:lines.join('\n')})});
...
export async function verifyTurnstile(request,env,token) {
  ...
  if(!response.ok) {
    ...
    let codes=body.slice(0,120);
    try{const parsed=JSON.parse(body);if(Array.isArray(parsed['error-codes']))codes=parsed['error-codes'].join(',');}catch{}
    console.error(JSON.stringify({event:'siteverify_failed',status:response.status,codes}));
    return json({error:'verification_unavailable',detail:codes},503);
  }
...
export function createBookingHandlers({readAvailability,verify=verifyTurnstile,now=()=>Math.floor(Date.now()/1000)}) {
  async function execute(context,mode) {
    ...
    try {
      ...
      const isProposal=booking.package==='event'||booking.package==='wedding';
      const agreement=isProposal
        ? {version:'proposal-2026-09-19',text:'Coverage terms are agreed at the in-person proposal meeting.'}
        : buildAgreement(booking);
      ...
      // Events have no signature step: the record is derived from the client's
      // name, and the consents below are implicit in the booking terms.
      const signature=isProposal
        ? {typedName:booking.details.name,agreementConsent:true,electronicConsent:true,promotion:null,drawing:[]}
        : normalizeSignature(input.signature,booking);
      ...
      if(!isProposal&&(typeof input.agreementHash!=='string'||!/^[a-f0-9]{64}$/.test(input.agreementHash)||typeof input.agreementVersion!=='string'||input.agreementVersion.length>100))throw new BookingError('agreement_required');
      if(!env.BOOKING_DB)throw new BookingError('storage_not_configured',503);
      const keyHash=await sha256(input.submissionKey);
      const payloadHash=await sha256(JSON.stringify({booking,signature,version:isProposal?agreement.version:input.agreementVersion,hash:isProposal?agreementHash:input.agreementHash}));
      ...
      if(!isProposal&&(input.agreementVersion!==agreement.version||input.agreementHash!==agreementHash))throw new BookingError('agreement_changed_review_again',409);
      await verify(request,env,input.turnstileToken);
      // Clean up any orphaned rows whose calendar events were deleted by the owner.
      if(env.BOOKING_DB){
        try{
          const staleRows=await env.BOOKING_DB.prepare('SELECT id FROM booking_requests').all();
          if(staleRows.success&&Array.isArray(staleRows.results)){
            for(const staleRow of staleRows.results){
              if(typeof staleRow.id!=='string'||!staleRow.id)continue;
              if(staleRow.id===input.submissionKey)continue;
              try{
                const searchResponse=await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(env.GOOGLE_BOOKINGS_CALENDAR_ID)}/events?privateExtendedProperty=requestId=${encodeURIComponent(staleRow.id)}`,{headers:{Authorization:`Bearer ${(await import('./google-auth.mjs')).accessToken(env)}`}});
                if(searchResponse.ok){
                  const searchData=await searchResponse.json();
                  const items=Array.isArray(searchData.items)?searchData.items:[];
                  if(!items.length){
                    await env.BOOKING_DB.prepare('DELETE FROM booking_jobs WHERE request_id=?').bind(staleRow.id).run();
                    await env.BOOKING_DB.prepare('DELETE FROM booking_requests WHERE id=?').bind(staleRow.id).run();
                  }
                }
              }catch(e){/* skip rows that fail to search */}
            }
          }
        }catch(e){}
      }
      // Weddings have no availability check: a date >=30 days out is sufficient.
      if(booking.package!=='wedding'){
      ...
      if(booking.package==='event'){
        if(available.busy?.some(([a,b])=>booking.sessionStart*1000-30000<b&&booking.sessionEnd*1000+30000>a))throw new BookingError('time_unavailable',409);
      }else if(!isProposal&&!available.days?.find(x=>x.key===booking.date)?.slots?.includes(booking.time)){
        throw new BookingError('time_unavailable',409);
      }
      }
      const createdAt=now();
      if(booking.sessionStart<createdAt+432000)throw new BookingError('time_unavailable',409);
      // Proposal-based bookings (events/weddings): replace any existing booking
      // for the same service+date, since the owner manages these via the calendar.
      if(isProposal){
        const dayStart=sessionSeconds(booking.date,0);
        const dayEnd=sessionSeconds(booking.date,1410)+1800;
        await db.prepare('DELETE FROM booking_jobs WHERE request_id IN (SELECT id FROM booking_requests WHERE service=? AND session_start>=? AND session_start<?)').bind(booking.service,dayStart,dayEnd).run();
        await db.prepare('DELETE FROM booking_requests WHERE service=? AND session_start>=? AND session_start<?').bind(booking.service,dayStart,dayEnd).run();
      }
      ...
      for(let attempt=0;attempt<5;attempt++){
        try {
          ...
          service,session_start,session_end,busy_start,busy_end,package_id,price_cents,customer_json,
      ...
      const eventFailure=await createBookingEvent(env,booking,id);
      if(eventFailure){
        // No calendar event means no booking: roll back the saved request so
        // the customer sees the failure and nothing is left orphaned.
        await db.prepare('DELETE FROM booking_jobs WHERE request_id=?').bind(id).run().catch(()=>{});
        await db.prepare('DELETE FROM booking_requests WHERE id=?').bind(id).run().catch(()=>{});
        console.error(JSON.stringify({event:'booking_rolled_back',reason:eventFailure.slice(0,60)}));
        throw new BookingError('calendar_unavailable',503);
      }
      await db.prepare(`UPDATE booking_requests SET status = 'approved', decision_at = unixepoch() WHERE id = ?`).bind(id).run();
