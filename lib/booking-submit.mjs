...
async function createBookingEvent(env,booking,id) {
  try {
    ...
    const serviceLabel=booking.package==='event'?'Event Coverage':PACKAGES[booking.package].name;
...
async function sendOwnerNotification(env,booking,receiptRow,holdNote='') {
  ...
  const d=booking.details,isEvent=booking.package==='event';
  const serviceLabel=isEvent?'Event Coverage':PACKAGES[booking.package].name;
...
export function createBookingHandlers({readAvailability,verify=verifyTurnstile,now=()=>Math.floor(Date.now()/1000)}) {
  async function execute(context,mode) {
    ...
    try {
      ...
      const isEvent=booking.package==='event';
      const agreement=isEvent
        ? {version:'event-proposal-2026-09-19',text:'Coverage terms are agreed at the in-person proposal meeting.'}
      ...
      if(!isEvent&&(typeof input.agreementHash!=='string'||!/^[a-f0-9]{64}$/.test(input.agreementHash)||typeof input.agreementVersion!=='string'||input.agreementVersion.length>100))throw new BookingError('agreement_required');
      if(!env.BOOKING_DB)throw new BookingError('storage_not_configured',503);
      const keyHash=await sha256(input.submissionKey);
      const payloadHash=await sha256(JSON.stringify({booking,signature,version:isEvent?agreement.version:input.agreementVersion,hash:isEvent?agreementHash:input.agreementHash}));
      ...
      if(!isEvent&&(input.agreementVersion!==agreement.version||input.agreementHash!==agreementHash))throw new BookingError('agreement_changed_review_again',409);
      ...
      if(isEvent){
        // Events: the chosen start must fit a clear window with buffers
      ...
      }else if(!available.days?.find(x=>x.key===booking.date)?.slots?.includes(booking.time)){
